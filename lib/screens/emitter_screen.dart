...
class _EmitterScreenState extends State<EmitterScreen> {
  ...
  void initState() {
    ...
    _flowLogService.logFlow(
      script: 'emitter_screen.dart - initState',
      message: 'Inicializando pantalla del emisor.',
    );
    ...
    _bluetoothService.connectionStatus.listen((isConnected) {
      if (_connectionMode == 'bluetooth') {
        // Solo actualizar si estamos en modo Bluetooth
        setState(() {
          _isConnected = isConnected;
          _updateConnectionStatusText(); // Usar el método para actualizar el texto
        });
        _flowLogService.logFlow(
          script: 'emitter_screen.dart - connectionStatusStream',
          message:
              'Estado de conexión Bluetooth actualizado: $_connectionStatusText.',
        );
      }
    });
    ...
    _notificationService.notificationStream.listen((notification) {
      _flowLogService.logFlow(
        script: 'emitter_screen.dart - notificationsStream',
        message:
            'Notificación recibida de NotificationService: ${notification.title}.',
      );
      ...
      if (_pairingCode != null && _pairingCode != 'Generando...') {
        _flowLogService.logFlow(
          script: 'emitter_screen.dart - notificationsStream',
          message:
              'Enviando notificación a Internet con pairing code: $_pairingCode.',
        );
        try {
          print(
            '[EmitterScreen] Enviando notificación a Internet: ${notification.toMap()}',
          ); // NUEVO PRINT
          _internetConnectionService.sendNotification(
            _pairingCode,
            notification,
          );
        } catch (e, st) {
          print(
            '[EmitterScreen] Error al enviar notificación a Internet: ${e.toString()}',
          ); // NUEVO PRINT
        ...
        _flowLogService.logFlow(
          script: 'emitter_screen.dart - notificationsStream',
          message:
              'No se pudo enviar la notificación a Internet. Pairing code no válido.',
        );
      }
  ...
  Future<void> _checkPermissions() async {
    try {
      _flowLogService.logFlow(
        ...
        message: 'Verificando permisos Bluetooth y notificaciones.',
      );
      ...
      final notificationListenerStatus =
      ...
      setState(() {
        ...
        _flowLogService.logFlow(
          script: 'emitter_screen.dart - _checkPermissions',
          message:
              'Permisos verificados. Bluetooth: $_bluetoothPermissionGranted, Notificaciones: $_notificationPermissionGranted',
        );
      });
    } catch (e, st) {
      _errorService.logError(
  ...
  Future<void> _requestPermissions() async {
    try {
      _flowLogService.logFlow(
        ...
        message: 'Solicitando permisos Bluetooth y notificaciones.',
      );
      ...
      if (notificationPermissionGranted) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text(
              'Se requiere acceso a las notificaciones para el funcionamiento correcto de la aplicación.',
            ),
      ...
      _flowLogService.logFlow(
        script: 'emitter_screen.dart - _requestPermissions',
        message: 'Solicitud de permisos completada.',
      );
    } catch (e, st) {
      _errorService.logError(
        script: 'emitter_screen.dart - _requestPermissions',
  ...
  Future<void> _loadConnectionMode() async {
    ...
    _flowLogService.logFlow(
      script: 'emitter_screen.dart - _loadConnectionMode',
      message: 'Modo de conexión cargado: $_connectionMode.',
    );
  }
  ...
  Future<void> _loadUniqueCode() async {
    ...
    if (code == null) {
      ...
      _flowLogService.logFlow(
        script: 'emitter_screen.dart - _loadUniqueCode',
        message: 'Código único generado y guardado: $code.',
      );
    } else {
      _flowLogService.logFlow(
        script: 'emitter_screen.dart - _loadUniqueCode',
        message: 'Código único cargado: $code.',
      );
    }
    setState(() {
      _pairingCode = code!;
  ...
  String _generateUniqueCode() {
    _flowLogService.logFlow(
      script: 'emitter_screen.dart - _generateUniqueCode',
      message: 'Generando código único de 6 dígitos.',
    );
    final random = Random();
    String code = '';
    for (int i = 0; i < 6; i++) {
  ...
  Widget build(BuildContext context) {
    _flowLogService.logFlow(
      script: 'emitter_screen.dart - build',
      message: 'Construyendo UI de la pantalla del emisor.',
    );
    return Scaffold(
      appBar: AppBar(title: const Text('Emisor')),
                    ...
                    Padding(
                      ...
                      child: Column(
                        ...
                        children: [
                              ...
                              Switch(
                                ...
                                onChanged: (value) async {
                                  setState(() {
                                    ...
                                    _connectionMode =
                                        value ? 'internet' : 'bluetooth';
                                    // Al cambiar de modo, asumimos que la conexión se pierde hasta que se establezca en el nuevo modo
                                    _isConnected =
                                        false; // Resetear estado de conexión
                                  });
                                  await _storageService.saveConnectionMode(
                                    _connectionMode,
                                  ...
                                  _flowLogService.logFlow(
                                    script: 'emitter_screen.dart - build',
                                    message:
                                        'Toggle de modo de conexión cambiado a: $_connectionMode.',
                                  );
                                  ...
                                  if (_connectionMode == 'internet') {
                                    ...
                                    _flowLogService.logFlow(
                                      script: 'emitter_screen.dart - build',
                                      message:
                                          'Cambiando a modo Internet. Iniciando emisor.',
                                    );
                                    if (_pairingCode != 'Generando...') {
                                      _flowLogService.logFlow(
                                        script: 'emitter_screen.dart - build',
                                        message:
                                            'Llamando a startEmitter con código: $_pairingCode',
                                      );
                                      await _internetConnectionService
                                          .startEmitter(_pairingCode);
                          ...
                          Center(
                            child: Row(
                              // Usar Row para alinear el texto y el indicador
                              ...
                              children: [
                                ...
                                if (!_isConnected)
                                  const SizedBox(
                                    width: 8,
                                  ), // Espacio entre indicador y texto
                                Text(
                          ...
                          ElevatedButton(
                            onPressed: () {
                              _flowLogService.logFlow(
                                script: 'emitter_screen.dart - build',
                                message:
                                    'Botón "Seleccionar Aplicaciones" presionado.',
                              );
                              Navigator.push(
                                context,
                                MaterialPageRoute(
                          ...
                          ElevatedButton(
                            onPressed: () {
                              _flowLogService.logFlow(
                                script: 'emitter_screen.dart - build',
                                message:
                                    'Botón "Solicitar Permisos de Notificación" presionado.',
                              );
                              _requestPermissions();
                            },
                            child: const Text(
