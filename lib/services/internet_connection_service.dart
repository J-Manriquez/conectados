...
class InternetConnectionService {
  ...
  final ConnectionDocumentService _connectionDocumentService =
      ConnectionDocumentService(); // Instancia del nuevo servicio
  ...
  Future<void> startEmitter(String uniqueCode) async {
    _flowLogService.logFlow(
      ...
      message: 'Iniciando modo emisor por Internet con código: $uniqueCode.',
    );
    try {
      ...
      _flowLogService.logFlow(
        script: 'internet_connection_service.dart - startEmitter',
        message:
            'Documento de conexión creado/actualizado con éxito para código: $uniqueCode.',
      );
      print(
        'Documento de conexión creado/actualizado con éxito para código: $uniqueCode',
      );
      ...
      _flowLogService.logFlow(
        script: 'internet_connection_service.dart - startEmitter',
        message: 'Error al iniciar modo emisor: ${e.toString()}',
      );
  ...
  Future<void> stopEmitter(String uniqueCode) async {
    _flowLogService.logFlow(
      ...
      message:
          'Intentando detener emisor por Internet con código: $uniqueCode.',
    );
    try {
      ...
      if (docSnapshot.exists) {
        ...
        _flowLogService.logFlow(
          script: 'internet_connection_service.dart - stopEmitter',
          message:
              'Estado de conexión por Internet actualizado a "disconnected" para código: $uniqueCode.',
        );
      } else {
        ...
        _flowLogService.logFlow(
          script: 'internet_connection_service.dart - stopEmitter',
          message:
              'Documento de conexión por Internet no encontrado para código: $uniqueCode. No se requiere actualización.',
        );
      }
      ...
      _flowLogService.logFlow(
        script: 'internet_connection_service.dart - stopEmitter',
        message: 'Error al detener emisor por Internet: ${e.toString()}',
      );
    }
  }
  ...
  Future<void> sendNotification(
    ...
    _flowLogService.logFlow(
      ...
      message:
          'Intentando enviar notificación por Internet: ${notification.title}.',
    );
    try {
      print(
        '[InternetConnectionService] Enviando notificación a Firestore: ${notification.toMap()}',
      ); // NUEVO PRINT
      // Usar el nuevo servicio para añadir la notificación al documento
      await _connectionDocumentService.addNotificationToDocument(
        uniqueCode,
        notification,
      );
      _flowLogService.logFlow(
        script: 'internet_connection_service.dart - sendNotification',
        message:
            'Notificación enviada por Internet a Firestore para código: $uniqueCode.',
      );
      ...
      _flowLogService.logFlow(
        script: 'internet_connection_service.dart - sendNotification',
        message: 'Error al enviar notificación por Internet: ${e.toString()}',
      );
  ...
  Stream<List<NotificationItem>>? connectReceiver(String uniqueCode) {
    _flowLogService.logFlow(
      ...
      message:
          'Intentando conectar como receptor por Internet con código: $uniqueCode.',
    );
    if (uniqueCode.isEmpty) {
      _flowLogService.logFlow(
        script: 'internet_connection_service.dart - connectReceiver',
        message: 'Código vacío proporcionado.',
      );
    ...
    final connectionDocumentStream = _connectionDocumentService
        .getConnectionDocumentStream(uniqueCode);
    ...
    docRef
        ...
        .then((docSnapshot) async {
          if (docSnapshot.exists) {
            _flowLogService.logFlow(
              script:
                  'internet_connection_service.dart - connectReceiver - get',
              message:
                  'Documento encontrado para código: $uniqueCode. Actualizando estado a "connected".',
            );
            // Usar el nuevo servicio para actualizar el estado
            await _connectionDocumentService.updateConnectionStatus(
              uniqueCode,
              'connected',
            );
            ...
            _flowLogService.logFlow(
              script:
                  'internet_connection_service.dart - connectReceiver - get',
              message: 'Documento no encontrado para código: $uniqueCode.',
            );
        ...
        .catchError((e, st) {
          _errorService.logError(
            script:
                'internet_connection_service.dart - connectReceiver - get/update',
            error: e,
            stackTrace: st,
          );
          _flowLogService.logFlow(
            script:
                'internet_connection_service.dart - connectReceiver - get/update',
            message:
                'Error al verificar/actualizar documento de conexión: ${e.toString()}',
          );
    ...
    return connectionDocumentStream.map((connectionDoc) {
      if (connectionDoc != null) {
        _flowLogService.logFlow(
          script: 'internet_connection_service.dart - connectReceiver',
          message:
              'Notificaciones recibidas por stream: ${connectionDoc.notifications.length}.',
        );
        ...
        _flowLogService.logFlow(
          script: 'internet_connection_service.dart - connectReceiver',
          message: 'Documento de conexión nulo en el stream.',
        );
  ...
  Future<void> disconnectReceiver(String uniqueCode) async {
    _flowLogService.logFlow(
      ...
      message: 'Desconectando receptor por Internet con código: $uniqueCode.',
    );
    try {
      ...
      _flowLogService.logFlow(
        script: 'internet_connection_service.dart - disconnectReceiver',
        message:
            'Estado actualizado a "disconnected" para código: $uniqueCode.',
      );
      ...
      _flowLogService.logFlow(
        script: 'internet_connection_service.dart - disconnectReceiver',
        message:
            'Error al desconectar receptor para código $uniqueCode: ${e.toString()}',
      );
